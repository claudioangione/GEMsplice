...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
        handles.legend = legend(bw_legend, 'location', 'best');
        legend boxoff;
    else
        handles.legend = [];
    end
    ...
    for i = 1:numbars
        ...
        handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none', 'linewidth', 0.5);
    ...
    if strcmp(legend_type, 'axis')
        for i = 1:numbars
            ...
            for j = 1:length(xdata)
                text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'HorizontalAlignment', 'right');
            end
        end
    ...
    set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'xtick',1:numgroups, 'linewidth', 1,'xgrid','off','ygrid','off');
